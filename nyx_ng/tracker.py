...
class ConnectionTracker(Daemon):
  ...
  def __init__(self, rate):
    ...
    if tor_controller().get_conf('DisableDebuggerAttachment', None) == '0':
      ...
      stem.util.log.notice("Tor connection information is unavailable. This is fine, but if you would like to have it please see https://github.com/Hero9774/nyx_NG/blob/master/README.md#no-connections")
  ...
  def get_value(self):
    ...
    :returns: **list** of :class:`~nyx_ng.tracker.Connection` we last
...
class ResourceTracker(Daemon):
  ...
  def get_value(self):
    ...
    :returns: latest :data:`~nyx_ng.tracker.Resources` we've polled
    """
...
class PortUsageTracker(Daemon):
  ...
  def fetch(self, port):
    ...
    :raises:
      * :class:`nyx_ng.tracker.UnresolvedResult` if the application is still
        being determined
      * :class:`nyx_ng.tracker.UnknownApplication` if the we tried to resolve
...
class ConsensusTracker(object):
  ...
  def __init__(self):
    ...
    cache_age = time.time() - nyx_ng.cache().relays_updated_at()
    controller = tor_controller()
  ...
  def get_relay_nickname(self, fingerprint):
    ...
    if not fingerprint:
      ...
      return nyx_ng.cache().relay_nickname(fingerprint)
  ...
  def get_relay_fingerprints(self, address):
    ...
    return nyx_ng.cache().relays_for_address(address)
  ...
  def get_relay_address(self, fingerprint, default):
    ...
    return nyx_ng.cache().relay_address(fingerprint, default)
