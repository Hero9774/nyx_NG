...
from nyx_ng import init_controller, uses_settings, nyx_interface
...
def main(config):
  ...
  try:
    args = nyx_ng.arguments.parse(sys.argv[1:])
    config.set('logged_events', args.logged_events)
    nyx_ng._config_path = args.config
  except ValueError as exc:
    print(exc)
    sys.exit(1)

  if args.print_help:
    print(nyx_ng.arguments.get_help())
    sys.exit()
  elif args.print_version:
    print(nyx_ng.arguments.get_version())
    sys.exit()
  ...
  nyx_ng.i18n.set_language(config.get('language', 'en'))

  if not config_exists:
    stem.util.log.notice('No nyx-ng configuration loaded, using defaults. You can customize nyx-ng by placing a configuration file at %s (see https://github.com/Hero9774/nyx_NG/blob/master/web/nyxrc.sample for its options).' % args.config)
  ...
  try:
    nyx_ng.curses.start(nyx_ng.draw_loop, acs_support = use_acs, transparent_background = True, cursor = False)
...
def _setup_debug_logging(args):
  ...
  stem.util.log.trace(DEBUG_HEADER.format(
    nyx_version = nyx_ng.__version__,
...
def _use_unicode(config):
  ...
  if config.get('unicode_support', True):
    ...
    if is_lang_unicode and nyx_ng.curses.is_wide_characters_supported():
...
def _shutdown_daemons(controller):
  ...
  halt_threads = [nyx_ng.tracker.stop_trackers()]
