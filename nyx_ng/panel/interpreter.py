...
from nyx_ng.curses import GREEN, MAGENTA, CYAN, BOLD, HIGHLIGHT
...
class InterpreterPanel(nyx_ng.panel.Panel):
  """
  ...
  def key_handlers(self):
    ...
    def _prompt_input():
      _scroll(nyx_ng.curses.KeyInput(curses.KEY_END))  # scroll to bottom
      self.redraw()

      return nyx_ng.curses.str_input(
        4 + self._x_offset,
      ...
      while self._is_input_mode:
        ...
        if response:
          for line in response.split('\n'):
            self._add_line([(text, attr) for text, attr in nyx_ng.curses.asci_to_curses(line)])

      self.redraw()

    return (
      nyx_ng.panel.KeyHandler('enter', 'execute a command', _start_input_mode, key_func = lambda key: key.is_selection()),
