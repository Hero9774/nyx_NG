...
import nyx_ng.arguments
import nyx_ng.curses
import nyx_ng.panel
import nyx_ng.popups
import nyx_ng.log

from nyx_ng import nyx_interface, tor_controller, join, input_prompt, show_message
from nyx_ng.curses import GREEN, YELLOW, WHITE, NORMAL, BOLD, HIGHLIGHT
...
class LogPanel(nyx_ng.panel.DaemonPanel):
  """
  ...
  def __init__(self):
    ...
    self._event_log = nyx_ng.log.LogGroup(CONFIG['max_log_size'])
    self._event_log_paused = None
    self._event_types = nyx_ng.log.listen_for_events(self._register_tor_event, logged_events)
    self._log_file = nyx_ng.log.LogFileOutput(CONFIG['write_logs_to'])
    self._filter = nyx_ng.log.LogFilters(initial_filters = CONFIG['logging_filter'])
    self._show_duplicates = not CONFIG['deduplicate_log']

    self._scroller = nyx_ng.curses.Scroller()
    self._has_new_event = False
    self._last_day = nyx_ng.log.day_count(time.time())

    # fetches past tor events from log file, if available

    if CONFIG['prepopulate_log']:
      log_location = nyx_ng.log.log_file_path(tor_controller())

      if log_location:
        try:
          for entry in reversed(list(nyx_ng.log.read_tor_log(log_location, CONFIG['prepopulate_read_limit']))):
  ...
  def _show_event_selection_prompt(self):
    ...
    event_types = nyx_ng.popups.select_event_types(self._event_types)

    if event_types and event_types != self._event_types:
      self._event_types = nyx_ng.log.listen_for_events(self._register_tor_event, event_types)
      self.redraw()
  ...
  def key_handlers(self):
    ...
    def _pick_filter():
      with nyx_ng.curses.CURSES_LOCK:
        ...
        selection = nyx_ng.popups.select_from_list('Log Filter:', options, initial_selection)
    ...
    return (
      ...
      nyx_ng.panel.KeyHandler('a', 'save snapshot of the log', self._show_snapshot_prompt),
      nyx_ng.panel.KeyHandler('e', 'change logged events', self._show_event_selection_prompt),
      nyx_ng.panel.KeyHandler('f', 'log regex filter', _pick_filter, 'enabled' if self._filter.selection() else 'disabled'),
      nyx_ng.panel.KeyHandler('u', 'duplicate log entries', _toggle_deduplication, 'visible' if self._show_duplicates else 'hidden'),
      nyx_ng.panel.KeyHandler('c', 'clear event log', _clear_log),
    )
  ...
  def _update(self):
    ...
    current_day = nyx_ng.log.day_count(time.time())
  ...
  def _register_tor_event(self, event):
    ...
    self._register_event(nyx_ng.log.LogEntry(event.arrived_at, event.type, msg))

  def _register_nyx_event(self, record):
    self._register_event(nyx_ng.log.LogEntry(int(record.created), 'NYX_%s' % record.levelname, record.msg))
...
def _draw_title(subwindow, event_types, event_filter):
  ...
  title_comp = list(nyx_ng.log.condense_runlevels(*event_types))
...
def _draw_entries(subwindow, x, y, event_log, show_duplicates):
  ...
  day_to_entries, today = {}, nyx_ng.log.day_count(time.time())
