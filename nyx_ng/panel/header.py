...
from nyx_ng.curses import RED, GREEN, YELLOW, CYAN, WHITE, BOLD, HIGHLIGHT
...
class HeaderPanel(nyx_ng.panel.DaemonPanel):
  """
  ...
  def __init__(self):
    ...
    self._last_width = nyx_ng.curses.screen_size().width
  ...
  def stop(self):
    ...
    nyx_ng.panel.DaemonPanel.stop(self)
  ...
  def show_message(self, message = None, *attr, **kwargs):
    ...
    :returns: :class:`~nyx_ng.curses.KeyInput` user pressed if provided a
    ...
    if 'max_wait' in kwargs:
      user_input = nyx_ng.curses.key_input(kwargs['max_wait'])
  ...
  def get_height(self):
    ...
    max_height = nyx_ng.panel.DaemonPanel.get_height(self)
  ...
  def key_handlers(self):
    def _reconnect():
      ...
      try:
        try:
          controller.reconnect(chroot_path = nyx_ng.chroot())
        except stem.connection.MissingPassword:
          password = nyx_ng.input_prompt('Controller Password: ')
    ...
    return (
      nyx_ng.panel.KeyHandler('n', action = self.send_newnym),
      nyx_ng.panel.KeyHandler('r', action = _reconnect),
    )
...
class Sampling(object):
  ...
  def create(last_sampling = None):
    ...
    tor_resources = nyx_ng.tracker.get_resource_tracker().get_value()
    ...
    my_router_status_entry = nyx_ng.tracker.get_consensus_tracker().my_router_status_entry()
