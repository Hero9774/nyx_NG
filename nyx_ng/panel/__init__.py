...
class KeyHandler(collections.namedtuple('Help', ['key', 'description', 'current'])):
  ...
  def handle(self, key):
    ...
    :param nyx_ng.curses.KeyInput key: keypress to be matched against
    """
...
class Panel(object):
  ...
  def __init__(self):
    ...
    self._last_draw_size = nyx_ng.curses.Dimensions(0, 0)
  ...
  def get_height(self):
    ...
    return max(0, nyx_ng.curses.screen_size().height - self._top)
  ...
  def key_handlers(self):
    ...
    :returns: **tuple** of :class:`~nyx_ng.panel.KeyHandler` instances
    """
  ...
  def submenu(self):
    ...
    :returns: :class:`~nyx_ng.menu.Submenu` with the panel's options or **None**
  ...
  def redraw(self, force = True, top = None):
    ...
    self._last_draw_size = nyx_ng.curses.draw(self._draw, top = self._top, height = self.get_height(), draw_if_resized = draw_dimension)
