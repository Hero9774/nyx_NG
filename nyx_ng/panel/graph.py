...
from nyx_ng import nyx_interface, tor_controller, join, show_message
from nyx_ng.curses import RED, GREEN, CYAN, BOLD, HIGHLIGHT
...
class ConnectionStats(GraphCategory):
  ...
  def bandwidth_event(self, event):
    ...
    for entry in nyx_ng.tracker.get_connection_tracker().get_value():
...
class ResourceStats(GraphCategory):
  ...
  def bandwidth_event(self, event):
    resources = nyx_ng.tracker.get_resource_tracker().get_value()
...
class GraphPanel(nyx_ng.panel.Panel):
  """
  Panel displaying graphical information of GraphCategory instances.
  """

  def __init__(self):
    nyx_ng.panel.Panel.__init__(self)
  ...
  def get_height(self):
    ...
    max_height = nyx_ng.panel.Panel.get_height(self)
      ...
      try:
        while True:
          ...
          if key.match('down'):
            ...
            max_height = nyx_ng.curses.screen_size().height - self.get_top()
  ...
  def key_handlers(self):
    def _pick_stats():
      ...
      selection = nyx_ng.popups.select_from_list('Graphed Stats:', options, previous_selection)
    ...
    def _pick_interval():
      self._update_interval = nyx_ng.popups.select_from_list('Update Interval:', list(Interval), self._update_interval)
      self.redraw()

    return (
      nyx_ng.panel.KeyHandler('g', 'resize graph', self._resize_graph),
      nyx_ng.panel.KeyHandler('s', 'graphed stats', _pick_stats, self._displayed_stat if self._displayed_stat else 'none'),
      nyx_ng.panel.KeyHandler('b', 'graph bounds', _next_bounds, self._bounds_type.replace('_', ' ')),
      nyx_ng.panel.KeyHandler('i', 'graph update interval', _pick_interval, self._update_interval),
    )
