...
from nyx_ng.country import get_country_name

from nyx_ng import nyx_interface, tor_controller
from nyx_ng.curses import WHITE, NORMAL, BOLD, HIGHLIGHT
from nyx_ng.menu import MenuItem, Submenu, RadioMenuItem, RadioGroup
...
class Entry(object):
  ...
  def get_lines(self):
    ...
    :returns: **list** of :class:`nyx_ng.panel.connection.Line` concerning this entry
    """
...
class ConnectionEntry(Entry):
  ...
  def _get_lines(self):
    ...
    if self.get_type() in (Category.OUTBOUND, Category.CIRCUIT, Category.DIRECTORY, Category.EXIT):
      fingerprint = nyx_ng.tracker.get_consensus_tracker().get_relay_fingerprints(self._connection.remote_address).get(self._connection.remote_port)

      if fingerprint:
        nickname = nyx_ng.tracker.get_consensus_tracker().get_relay_nickname(fingerprint)
  ...
  def _is_private(self):
    ...
    if self.get_type() == Category.INBOUND:
      return len(nyx_ng.tracker.get_consensus_tracker().get_relay_fingerprints(self._connection.remote_address)) == 0
...
class CircuitEntry(Entry):
  ...
  def _get_lines(self):
    def line(fingerprint, line_type):
      ...
      consensus_tracker = nyx_ng.tracker.get_consensus_tracker()
      ...
      connection = nyx_ng.tracker.Connection(datetime_to_unix(self._circuit.created), False, '127.0.0.1', 0, address, port, 'tcp', False)
...
class ConnectionPanel(nyx_ng.panel.DaemonPanel):
  """
  ...
  def __init__(self):
    nyx_ng.panel.DaemonPanel.__init__(self, UPDATE_RATE)

    self._scroller = nyx_ng.curses.CursorScroller()
  ...
  def _show_sort_dialog(self):
    ...
    results = nyx_ng.popups.select_sort_order('Connection Ordering:', SortAttr, self._sort_order, sort_colors)
  ...
  def key_handlers(self):
    ...
    def _show_descriptor():
      ...
      while True:
        ...
        key = nyx_ng.popups.show_descriptor(selected.fingerprint, color, is_close_key)

        if not key or key.is_selection() or key.match('d'):
          break  # closes popup
        elif key.match('left'):
          _scroll(nyx_ng.curses.KeyInput(curses.KEY_UP))
        elif key.match('right'):
          _scroll(nyx_ng.curses.KeyInput(curses.KEY_DOWN))

      self.redraw()

    def _pick_connection_resolver():
      connection_tracker = nyx_ng.tracker.get_connection_tracker()
      resolver = connection_tracker.get_custom_resolver()
      options = ['auto'] + list(connection.Resolver) + list(nyx_ng.tracker.CustomResolver)

      selected = nyx_ng.popups.select_from_list('Connection Resolver:', options, resolver if resolver else 'auto')
      connection_tracker.set_custom_resolver(None if selected == 'auto' else selected)

      self.redraw()

    def _show_client_locales():
      nyx_ng.popups.show_counts('Client Locales', self._client_locale_usage)
    ...
    def _show_exiting_port_usage():
      ...
      nyx_ng.popups.show_counts('Exiting Port Usage', counts)

    resolver = nyx_ng.tracker.get_connection_tracker().get_custom_resolver()
    user_traffic_allowed = tor_controller().is_user_traffic_allowed()

    options = [
      nyx_ng.panel.KeyHandler('arrows', 'scroll up and down', _scroll, key_func = lambda key: key.is_scroll()),
      nyx_ng.panel.KeyHandler('enter', 'show connection details', _show_details, key_func = lambda key: key.is_selection()),
      nyx_ng.panel.KeyHandler('d', 'raw consensus descriptor', _show_descriptor),
      nyx_ng.panel.KeyHandler('s', 'sort ordering', self._show_sort_dialog),
      nyx_ng.panel.KeyHandler('r', 'connection resolver', _pick_connection_resolver, 'auto' if resolver is None else resolver),
    ]

    if user_traffic_allowed.inbound:
      options.append(nyx_ng.panel.KeyHandler('c', 'client locale usage summary', _show_client_locales))

    if user_traffic_allowed.outbound:
      options.append(nyx_ng.panel.KeyHandler('e', 'exit port usage summary', _show_exiting_port_usage))
  ...
  def submenu(self):
    ...
    tracker = nyx_ng.tracker.get_connection_tracker()
  ...
  def _update(self):
    ...
    conn_resolver = nyx_ng.tracker.get_connection_tracker()
    ...
    if CONFIG['resolve_processes']:
      ...
      nyx_ng.tracker.get_port_usage_tracker().query(local_ports, remote_ports)
...
def _draw_address_column(subwindow, x, y, line, attr):
  ...
  if line.line_type == LineType.CIRCUIT_HEADER and line.circuit.status != 'BUILT':
    ...
    if line.entry.get_type() == Category.EXIT:
      ...
      locale_str = get_country_name(locale_upper) if locale_upper else '??'
      dst += ' (%s)' % locale_str
...
def _draw_details(subwindow, selected):
  ...
  if selected.line_type == LineType.CIRCUIT_HEADER and selected.circuit.status != 'BUILT':
    ...
    matches = nyx_ng.tracker.get_consensus_tracker().get_relay_fingerprints(selected.connection.remote_address)
...
def _draw_line_details(subwindow, x, y, line, width, attr):
  if line.line_type == LineType.CIRCUIT_HEADER:
    ...
    try:
      ...
      process = nyx_ng.tracker.get_port_usage_tracker().fetch(port)
      comp = ['%s (%s)' % (process.name, process.pid) if process.pid else process.name]
    except nyx_ng.tracker.UnresolvedResult:
      comp = ['resolving...']
    except nyx_ng.tracker.UnknownApplication:
      comp = ['UNKNOWN']
  else:
