...
"""
Panel presenting the configuration state for tor or nyx_ng. Options can be edited
...
from nyx_ng import tor_controller, input_prompt, show_message
from nyx_ng.curses import WHITE, NORMAL, BOLD, HIGHLIGHT
from nyx_ng.menu import MenuItem, Submenu
...
class ConfigPanel(nyx_ng.panel.Panel):
  """
  ...
  def _show_sort_dialog(self):
    ...
    results = nyx_ng.popups.select_sort_order('Config Option Ordering:', SortAttr, self._sort_order, sort_colors)
  ...
  def _show_write_dialog(self):
    ...
    if nyx_ng.popups.confirm_save_torrc(torrc):
      try:
  ...
  def key_handlers(self):
    ...
    return (
      ...
      nyx_ng.panel.KeyHandler('enter', 'edit configuration option', _edit_selected_value, key_func = lambda key: key.is_selection()),
      nyx_ng.panel.KeyHandler('w', 'write torrc', self._show_write_dialog),
      nyx_ng.panel.KeyHandler('a', 'toggle filtering', _toggle_show_all),
