...
def key_input(input_timeout = None):
  ...
  :returns: :class:`~nyx_ng.curses.KeyInput` that was pressed
  """
...
class _TextBacklog(object):
  """
  History backlog that allows the :func:`~nyx_ng.curses.str_input` function to
...
def screen_size():
  ...
  :returns: :data:`~nyx_ng.curses.Dimensions` with our screen size
  """
...
def get_color_override():
  ...
  :returns: :data:`~nyx_ng.curses.Color` for the color requrests will be
...
def set_color_override(color = None):
  ...
  :param nyx_ng.curses.Color color: color to override all requests with, **None**
...
def draw(func, left = 0, top = 0, width = None, height = None, background = None, draw_if_resized = None):
  ...
  :class:`~nyx_ng.curses._Subwindow`.
  ...
  :param nyx_ng.curses.Color background: background color, unset if **None**
  :param nyx_ng.curses.Dimension draw_if_resized: only draw content if
    dimentions have changed from this

  :returns: :class:`~nyx_ng.curses.Dimension` for the space we drew within
  """
...
class Scroller(object):
  ...
  def handle_key(self, key, content_height, page_height):
    ...
    :param nyx_ng.curses.KeyInput key: pressed key
    :param int content_height: height of the content being renered
    :param int page_height: height visible on the page
