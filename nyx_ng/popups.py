...
from nyx_ng import nyx_interface
from nyx_ng.curses import RED, GREEN, YELLOW, CYAN, WHITE, NORMAL, BOLD, HIGHLIGHT
...
def show_help():
  ...
  :returns: :class:`~nyx_ng.curses.KeyInput` that was pressed to close the popup
  ...
  with nyx_ng.curses.CURSES_LOCK:
    nyx_ng.curses.draw(_render, top = _top(), width = 80, height = 9)
    keypress = nyx_ng.curses.key_input()
...
def show_about():
  ...
  def _render(subwindow):
    ...
    subwindow.addstr(2, 1, 'Nyx NG, version %s (released %s)' % (nyx_ng.__version__, nyx_ng.__release_date__), BOLD)
    subwindow.addstr(4, 2, 'Fork by H.Ommen, based on nyx by Damian Johnson')
    subwindow.addstr(4, 3, 'Project page: %s' % nyx_ng.__url__)
    subwindow.addstr(2, 5, 'Released under the GPL v3 (http://www.gnu.org/licenses/gpl.html)')
    subwindow.addstr(2, 7, 'Press any key...')

  with nyx_ng.curses.CURSES_LOCK:
    nyx_ng.curses.draw(_render, top = _top(), width = 80, height = 9)
    nyx_ng.curses.key_input()
...
def show_counts(title, counts, fill_char = ' '):
  ...
  with nyx_ng.curses.CURSES_LOCK:
    if not counts:
      nyx_ng.curses.draw(_render_no_stats, top = _top(), width = len(NO_STATS_MSG) + 4, height = 3)
    else:
      nyx_ng.curses.draw(_render_stats, top = _top(), width = 80, height = 4 + max(1, len(counts)))
...
def show_descriptor(fingerprint, color, is_close_key):
  ...
  :returns: :class:`~nyx_ng.curses.KeyInput` for the keyboard input that
    closed the dialog
  """
  ...
  scroller = nyx_ng.curses.Scroller()
  ...
  screen_size = nyx_ng.curses.screen_size()
  ...
  with nyx_ng.curses.CURSES_LOCK:
    nyx_ng.curses.draw(lambda subwindow: subwindow.addstr(0, 0, ' ' * 500), top = _top(), height = 1)  # hides title below us
    nyx_ng.curses.draw(_render, top = _top(), width = width, height = height)
    popup_height = min(screen_size.height - _top(), height)

    while True:
      key = nyx_ng.curses.key_input()
...
def _descriptor_text(fingerprint):
  ...
  controller = nyx_ng.tor_controller()
...
def select_from_list(title, options, previous_selection):
  ...
  with nyx_ng.curses.CURSES_LOCK:
    while True:
      nyx_ng.curses.draw(lambda subwindow: subwindow.addstr(0, 0, ' ' * 500), top = _top(), height = 1)  # hides title below us
      nyx_ng.curses.draw(_render, top = _top(), width = max(map(len, options)) + 9, height = len(options) + 2)
      key = nyx_ng.curses.key_input()
...
def select_event_types(initial_selection):
  ...
  event_names = nyx_ng.tor_controller().get_info('events/names', None)
  ...
  events = [event for event in event_names.split() if event not in nyx_ng.log.TOR_RUNLEVELS]
  ...
  def _render(subwindow):
    ...
    if selection < 10:
      description = stem.control.event_description(nyx_ng.log.TOR_RUNLEVELS[selection % 5])
    ...
    for i, event in enumerate(nyx_ng.log.TOR_RUNLEVELS):
    ...
    for i, event in enumerate(nyx_ng.log.NYX_RUNLEVELS):
      x = subwindow.addstr(x + 4, 5, '[X]' if event in selected_events else '[ ]')
      x = subwindow.addstr(x + 1, 5, nyx_ng.log.TOR_RUNLEVELS[i], HIGHLIGHT if selection == (i + 5) else NORMAL)
  ...
  with nyx_ng.curses.CURSES_LOCK:
    while True:
      nyx_ng.curses.draw(_render, top = _top(), width = 80, height = (len(events) // 3) + 10)
      key = nyx_ng.curses.key_input()
      ...
      if key.match('up'):
        ...
        if selection < 5:
          selected_event = nyx_ng.log.TOR_RUNLEVELS[selection]
        elif selection < 10:
          selected_event = nyx_ng.log.NYX_RUNLEVELS[selection - 5]
        elif selection == len(events) + 10:
...
def confirm_save_torrc(torrc):
  ...
  with nyx_ng.curses.CURSES_LOCK:
    while True:
      nyx_ng.curses.draw(_render, top = _top(), height = len(torrc_lines) + 2)
      key = nyx_ng.curses.key_input()
