...
class HeaderWorker(QThread):
    ...
    def _collect(self):
        ...
        try:
            ct = nyx_ng.tracker.get_consensus_tracker()
...
class LogWorker(QThread):
    ...
    log_entry = pyqtSignal(object)  # nyx_ng.log.LogEntry
    ...
    def run(self):
        ...
        while not _NYX_LOG_BUFFER.empty():
            try:
                ...
                self.log_entry.emit(nyx_ng.log.LogEntry(
                    int(record.created),
        ...
        try:
            ...
            if log_location:
                for entry in reversed(list(nyx_ng.log.read_tor_log(log_location))):
                    if not self._halt:
        ...
        def _on_log(event):
            ...
            if isinstance(event, stem.response.events.LogEvent):
                msg = event.message
            self.log_entry.emit(nyx_ng.log.LogEntry(event.arrived_at, event.type, msg))

        self._callback = _on_log
        nyx_ng.log.listen_for_events(self._callback, self._events)
