...
from nyx_ng import nyx_interface, tor_controller, show_message
from nyx_ng.curses import RED, WHITE, NORMAL, BOLD, UNDERLINE
...
def show_menu():
  ...
  with nyx_ng.curses.CURSES_LOCK:
    ...
    while not cursor.is_done:
      ...
      cursor.handle_key(nyx_ng.curses.key_input())
...
def _make_menu():
  ...
  root_menu.add(Submenu('Help', [
    MenuItem('Hotkeys', nyx_ng.popups.show_help),
    MenuItem('About', nyx_ng.popups.show_about),
  ]))
...
def _language_menu():
  lang_group = RadioGroup(nyx_ng.set_language, nyx_ng.i18n.get_language())
...
def _view_menu():
  ...
  if nyx_ng.curses.is_color_supported():
    color_group = RadioGroup(nyx_ng.curses.set_color_override, nyx_ng.curses.get_color_override())

    view_menu.add(Submenu('Color', [
      RadioMenuItem('All', color_group, None),
      [RadioMenuItem(str_tools._to_camel_case(opt), color_group, opt) for opt in nyx_ng.curses.Color],
    ]))
...
def _draw_top_menubar(menu, selection):
  ...
  nyx_ng.curses.draw(_render, height = 1, background = RED)
...
def _draw_submenu(selection, submenu, top, left):
  ...
  nyx_ng.curses.draw(_render, top = top, left = left, width = menu_width, height = len(submenu.children), background = RED)
