...
try:
  ...
  for cmd, stem_install in PACKAGE_MANAGERS.items():
    if shutil.which(cmd):
      print("nyx-ng requires stem, try running '%s'" % stem_install)
      sys.exit(1)

  print('nyx-ng requires stem, you can find it at https://stem.torproject.org/download.html')
  sys.exit(1)
...
don't bundle this with the interpreter. Please let us know at...
  ...
  https://github.com/Hero9774/nyx_NG/issues/
"""
...
def draw_loop():
  ...
  for panel in interface:
    if isinstance(panel, nyx_ng.panel.DaemonPanel):
      panel.start()
  ...
  while not interface._quit:
    if next_key:
      ...
      key = nyx_ng.curses.key_input(CONFIG['redraw_rate'])
    ...
    if key.match('right'):
      ...
      nyx_ng.menu.show_menu()
    elif key.match('q'):
      if CONFIG['confirm_quit']:
        confirmation_key = show_message('Are you sure (q again to confirm)?', nyx_ng.curses.BOLD, max_wait = 30)
      ...
      confirmation_key = show_message("This will reset Tor's internal state. Are you sure (x again to confirm)?", nyx_ng.curses.BOLD, max_wait = 30)
      ...
      next_key = nyx_ng.popups.show_help()
...
def nyx_interface():
  ...
  :returns: :class:`~nyx_ng.Interface` controller
  """
...
def cache():
  ...
  :returns: :class:`~nyx_ng.cache.Cache` for our applicaion
  """
...
def input_prompt(msg, initial_value = ''):
  ...
  user_input = nyx_ng.curses.str_input(len(msg), header_panel.get_height() - 1, initial_value)
...
def init_controller(*args, **kwargs):
  """
  Sets the Controller used by nyx_ng. This is a passthrough for Stem's
...
def data_directory(filename, config):
  ...
  if not os.path.exists(data_dir):
    try:
      ...
      stem.util.log.log_once('nyx_ng.data_directory_unavailable', msg_runlevel, 'Unable to create a data directory at %s (%s). This is fine, but caching is disabled meaning performance will be diminished a bit.' % (data_dir, exc))
      return None
...
class Cache(object):
  ...
  def __init__(self):
    ...
    cache_path = nyx_ng.data_directory('cache.sqlite')
  ...
  def write(self):
    ...
    :returns: :class:`~nyx_ng.CacheWriter` that can modify the cache
    """
...
class Interface(object):
  ...
  def __init__(self):
    ...
    self._header_panel = nyx_ng.panel.header.HeaderPanel()
    first_page_panels = []

    if CONFIG['show_graph']:
      first_page_panels.append(nyx_ng.panel.graph.GraphPanel())

    if CONFIG['show_log']:
      first_page_panels.append(nyx_ng.panel.log.LogPanel())

    if first_page_panels:
      self._page_panels.append(first_page_panels)

    if CONFIG['show_connections']:
      self._page_panels.append([nyx_ng.panel.connection.ConnectionPanel()])

    if CONFIG['show_config']:
      self._page_panels.append([nyx_ng.panel.config.ConfigPanel()])

    if CONFIG['show_torrc']:
      self._page_panels.append([nyx_ng.panel.torrc.TorrcPanel()])

    if CONFIG['show_interpreter']:
      self._page_panels.append([nyx_ng.panel.interpreter.InterpreterPanel()])
  ...
  def header_panel(self):
    ...
    :returns: :class:`~nyx_ng.panel.header.HeaderPanel` of our interface
    """
  ...
  def halt(self):
    ...
    def halt_panels():
      daemons = [panel for panel in self if isinstance(panel, nyx_ng.panel.DaemonPanel)]
...
import nyx_ng.panel.connection
import nyx_ng.panel.graph
